...
class CataData():
    ...
    def __init__(
        ...
        """
        self.catalogue_paths = catalogue_paths if type(catalogue_paths) is list else [catalogue_paths]
    ...
    def cutout(self, coords: np.ndarray, field: str | int, return_wcs: bool = False) -> tuple | np.ndarray:
    ...
    def plot(self, index: int) -> None:
        ...
        plt.colorbar()
        plt.contour(image, levels=[self.df.iloc[index]["ISL_RMS"]*(3+n) for n in range(3)], origin='lower')
        plt.plot((self.cutout_width//2, self.cutout_width//2), (0, self.cutout_width-1), color="red", linewidth=2, ls="--", alpha=crosshair_alpha)
        plt.plot((0, self.cutout_width-1), (self.cutout_width//2, self.cutout_width//2), color="red", linewidth=2, ls="--", alpha=crosshair_alpha)
    ...
    def _build_df(self) -> pd.DataFrame:
        ...
        for catalogue_path, field in zip(self.catalogue_paths, self.field_names):
            data, wcs = self.open_fits(
                path=catalogue_path, 
                index=self.fits_index_catalogue
            )
    ...
    def _build_images(self, drop_axes: list[int]) -> tuple[dict[str | int, np.ndarray], dict[str | int, any]]:
    ...
    def _verify_input_lengths(self) -> None:
        ...
        """
        if len(self.image_paths) != len(self.catalogue_paths) != len(self.field_names)!= len(self.image_paths):
            raise ValueError(f"""Paths and fields must have same number of entries. Currently there are {len(self.image_paths)} image_paths, {len(self.catalogue_paths)} catalogue_paths, {len(self.field_names)} field_names""")
        return
