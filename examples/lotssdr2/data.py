...
class CustomTransform(torch.nn.Module):
    def __init__(self, transformations=[], range_normalize=False):
        ...
        # define jitter
        def cust_jitter(x):
            return x + torch.randn(x.size(), device=x.device, dtype=x.dtype) * 0.1
...
class LoTTS_Preprocessing:
    def __init__(
        ...
        scaling: float = 1.2,
        square: bool = False,
        cutout_size_name: list = ["cutout_height", "cutout_width"],
    ):
    ...
    def __call__(self, df):
        ...
        df[[*self.cutout_size_name]] = pd.DataFrame(
            [[None, None]] * len(df), index=df.index
        )

        # Get sizes from measurements (composite measurements)
        df.loc[:, "Composite_PA"] += 90
        composite_sizes = cata2data.preprocessing.calculate_cutout_size(
            df,
            scaling=self.scaling,
            square=self.square,
            cutout_size_name=self.cutout_size_name,
            major_axis="Composite_Width",
            minor_axis="Composite_Size",
            angle="Composite_PA",
        )
        ...
        df.loc[:, "PA"] += 90
        dc_sizes = cata2data.preprocessing.calculate_cutout_size(
            df,
            scaling=self.scaling,
            square=self.square,
            major_axis="Maj",
...
def LoTTSDataset(
    ...
    print(
        f"Found {len(image_paths)} images and {len(catalogue_paths)} catalogues in {data_folder}. Using {len(field_names)} fields."
    )
