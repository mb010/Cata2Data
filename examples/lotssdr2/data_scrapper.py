...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download data from the LOFAR surveys website"
    )
    parser.add_argument(
        "--url",
        help="The URL of the page to scrape",
        default="https://lofar-surveys.org/dr2_release.html",
    )
    parser.add_argument(
        "--base_url",
        help="The base URL of the website",
        default="https://lofar-surveys.org/",
    )
    parser.add_argument(
        "--download_link",
        help="The regex pattern to match the download link",
        default="^public/DR2/mosaics/.*/mosaic-blanked.fits$",
    )
    parser.add_argument(
        "--test", action="store_true", help="Only try to download the first file"
    )
    parser.add_argument(
        "--dir", help="The local directory in which to save the data.", default="./data"
    )
